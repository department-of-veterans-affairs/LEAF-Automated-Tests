...
(1,	'Configuration',	'',	'',	'',	NULL,	'leaf_agent',	NULL,	NULL,	NULL,	NULL,	0,	-128,	'2025-05-06 17:54:23',	0,	0,	1),
(2,	'Site',	'text',	'',	'',	1,	'leaf_agent',	NULL,	NULL,	NULL,	NULL,	0,	-128,	'2025-05-06 18:06:42',	0,	0,	1),
(3,	'Step ID',	'number',	'',	'',	1,	'leaf_agent',	NULL,	NULL,	NULL,	NULL,	0,	-127,	'2025-05-06 18:10:14',	0,	0,	1),
(4,	'Instructions',	'textarea',	'',	'',	NULL,	'leaf_agent',	NULL,	'<script src=\"js/agentViewer.js\"></script>\n<script>\nasync function main{{ iID }}() {\n    document.querySelector(\'#data_4_1\').style.display = \'none\';\n  \n  	let data = {};\n  	try {\n      	let task = document.querySelector(\'#data_4_1\').innerHTML;\n        if(task != \'\') {\n            data = JSON.parse(task);\n        }\n    } catch(err) {\n      	console.error(\'Task parsing error: \' + err);\n      	return;\n    }\n  \n  	let query = new LeafFormQuery();\n    query.addTerm(\'recordID\', \'=\', {{ recordID }});\n  	query.getData(2);\n  	let res = await query.execute();\n    let siteURL = res[{{ recordID }}].s1.id2;\n  \n  	let config = new LeafAgentViewer(\'agentConfig\', siteURL);\n    await config.init();\n  	config.importConfig(data);\n  	config.render();\n}\n\n// Wait for the page to fully load before running our function\nif(document.readyState !== \'loading\') {\n    main{{ iID }}();\n} else {\n    document.addEventListener(\'DOMContentLoaded\', main{{ iID }});\n}\n</script>\n\n<div id=\"agentConfig\"></div>',	NULL,	NULL,	0,	-127,	'2025-05-06 18:09:02',	0,	0,	1),
...
(8,	'Average Duration (ms)',	'number',	'',	'',	7,	'leaf_agent',	NULL,	NULL,	NULL,	NULL,	0,	-127,	'2025-06-16 19:07:24',	0,	0,	0),
(9,	'Log',	'raw_data',	'',	'',	7,	'leaf_agent',	NULL,	'<div id=\"grid{{ iID }}\"></div>\n<script>\nasync function main{{ iID }}() {\n  	let data = {};\n  	try {\n      	let log = document.querySelector(\'#data_9_1\').value;\n        if(log != \'\') {\n            data = JSON.parse(log);\n        }\n    } catch(err) {\n      	console.error(\'Task parsing error: \' + err);\n      	return;\n    }\n\n  	let grid = new LeafFormGrid(\'grid{{ iID }}\');\n  	grid.enableToolbar();\n  	grid.setDataBlob(data);\n    grid.setHeaders([\n        {name: \'Timestamp\', indicatorID: \'timestamp\', editable: false, callback: function(data, blob) {\n          	let timestamp = new Date(blob[data.recordID].timestamp * 1000);\n          	document.querySelector(\'#\'+data.cellContainerID).innerHTML = timestamp.toLocaleString();\n        }},\n        {name: \'Duration (ms)\', indicatorID: \'duration\', editable: false, callback: function(data, blob) {\n          	document.querySelector(\'#\'+data.cellContainerID).innerHTML = blob[data.recordID].duration;\n        }},\n        {name: \'Error\', indicatorID: \'error\', editable: false, callback: function(data, blob) {\n          	if(blob[data.recordID].errors != undefined) {\n              	let errors = \'\';\n              	let uid = \'\';\n                blob[data.recordID].errors.forEach(error => {\n                  	if(error.recordID > 0) {\n                      	uid = `UID# ${error.recordID}`;\n                    } else {\n                      	uid = \'SYSTEM\';\n                    }\n                	errors += `${uid}: ${error.error}<br />`;\n                });\n          		document.querySelector(\'#\'+data.cellContainerID).innerHTML = errors;\n            }\n        }}\n    ]);\n  	grid.sort(\'timestamp\', \'desc\');\n  	grid.renderBody();\n}\n\n// Wait for the page to fully load before running our function\nif(document.readyState !== \'loading\') {\n    main{{ iID }}();\n} else {\n    document.addEventListener(\'DOMContentLoaded\', main{{ iID }});\n}\n</script>',	NULL,	NULL,	0,	-126,	'2025-06-16 19:41:27',	0,	0,	0);
