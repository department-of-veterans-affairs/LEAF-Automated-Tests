...
test('Shortened URL Redirect', async ({ page }) => {
  ...
  await page.goto('https://host.docker.internal/Test_Request_Portal/?a=reports&v=3&query=N4IgLgpgTgtgziAXAbVASwCZJHSAHASQBEQAaEAez2gEMwKpsBCAXjJBjoGMALbKCHAoAbAG4Qs5AOZ0I2AIIA5EgF9S6LIhAYIwiJEmVqUOg2xtynMLyQAGabIXKQKgLrkAVhTQA7BChxoUTQuOXIuWSkGAE9FGhgwnDA6AFcEchouMDQKHwB9HjRcGPZcCDwAMRThADM0YWEEnzAAeR9haJB3HAYwJGA1EGE0GDQ%2BxABGW2nyYdHWmpq4fTsVIA%3D%3D%3D&indicators=NobwRAlgdgJhDGBDALgewE4EkAiYBcYyEyANgKZgA0YUiAthQVWAM4bL4AMAvpeNHCRosuAizLoAbggrVaDfGGZt0HPDz6RYCFBhyLkATwAOsmvUZLqKtRv7ahe0a2QoAri2bzLy9l172groiiojwRKhQAEJuyGhQXhaKvqr%2BmgI6wvoEMIiGLAD6LNDwZAUkiCzIBWERCXJJTNZ%2B6twAukA');
  ...
  await page.goto('https://host.docker.internal/Test_Request_Portal/api/open/form/query/_32wmX?x-filterData=recordID,title,service,categoryNames,stepTitle,lastStatus,action_history.time,action_history.stepID,action_history.actionType,stepFulfillmentOnly,submitted');
  await page.locator('div').click();
  await expect(page.getByText('{ "5": { "recordID": 5, "')).toBeVisible();
});
